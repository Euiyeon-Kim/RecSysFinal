...
def topk_eval(args, model, train_data, test_data, writer, epoch):
...
def train(config, datasets, writer, device):
    train_data, valid_data, test_data, model_define_args = datasets
    ...
    for epoch in range(config.n_epochs):
        ...
        if epoch % config.validate_freq == 0:
            # Evaluate every epoch
            eval_auc, eval_f1 = ctr_eval(config, model, valid_data)
            test_auc, test_f1 = ctr_eval(config, model, test_data)
            ...
            if config.topK:
                topk_eval(config, model, train_data, test_data, writer, epoch)
